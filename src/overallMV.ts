...
export const processOverallMV = async (week: number): Promise<void> => {
  await db.transaction().execute(async (trx) => {
    ...
    await sql`lock tables ${overallMV} write, ${overallMV} as O write, ${overallMV} as O2 write, ${systemValues} as SV read, ${picks} read, ${picks} as P read, ${games} read, ${games} as G read, ${games} as G2 read, ${games} as G3 read, ${users} read, ${users} as U read`
      .execute(trx);
      ...
      .whereExists((qb) =>
        qb.selectFrom((qb) =>
          qb.selectFrom("OverallMV as O2").select([sql.literal(1).as("Found")])
            .whereRef("O2.PointsEarned", ">", "O.PointsPossible").as("INNER")
        ).select("INNER.Found")
      ).execute();
